      ...
      <div className="flex flex-wrap items-center gap-1">
        {tags.map((label) => {
          ...
          return (
            <span
              key={label}
              className={
                "inline-flex items-center gap-0.5 rounded-sm border font-mono text-[11px] whitespace-nowrap " +
        ...
        {canEdit && !adding && (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-sm border border-dashed border-border/60 px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground transition-colors hover:border-[var(--neon-cyan)]/45 hover:text-foreground"
          >
            ...
            {suggestions.length === 0 ? (
              ...
              suggestions.map((label) => (
                <button
                  ...
                  disabled={pending}
                  className={
                    "rounded-sm border px-1.5 py-0.5 font-mono text-[11px] whitespace-nowrap transition-opacity hover:opacity-80 disabled:opacity-40 " +
                    linkTagToneClass(label)
                  }
                >
                  + {label}
      ...
      {readState && (
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            ...
            className={
              "inline-flex items-center gap-1 rounded-sm border px-1.5 py-0.5 font-mono text-[11px] whitespace-nowrap transition-colors disabled:opacity-50 " +
              (readState.read
          ...
          {readState.members > 0 &&
            (readState.unread === 0 ? (
              <span className="font-mono text-[12px] whitespace-nowrap text-emerald-200/70">
                {m.linkCard.allRead}
              </span>
            ) : (
              <span
                className="font-mono text-[12px] whitespace-nowrap text-amber-200/80"
                title={
