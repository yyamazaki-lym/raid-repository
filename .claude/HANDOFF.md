...
| # | 項目 | 規模 |
|---|---|---|
| 2 | スケジュール表自前実装 — **phase 1 / 2-A / 2-B / 2-C 完了 (2026-05-07)**: 3 モード切替 (`sync` / `native` / `disabled`) インフラ + native 用テーブル 3 件 schema + 8 server actions + self-row RLS + native トップ UI (admin 候補日追加 dialog + 本人出欠入力 popover + admin status toggle) + native settings UI (members CRUD + 凡例 CSV editor + CANCELLED 復帰)。**残 phase**: 3 = Discord 通知、4 = リマインダー cron + 連携。設計は `.claude/plans/todo-dazzling-locket.md` (全体) / `todo-2-phase-2-virtual-panda.md` (2-A) / `todo-2-phase-2-b-resilient-seal.md` (2-B) / `todo-2-phase-2-c-enchanted-kurzweil.md` (2-C) | 中 |
