...
| # | 項目 | 規模 |
|---|---|---|
| 2 | スケジュール表自前実装 — **phase 1 / 2-A / 2-B 完了 (2026-05-07)**: 3 モード切替 (`sync` / `native` / `disabled`) インフラ + native 用テーブル 3 件 schema + 8 server actions + self-row RLS + native UI 第 1 弾 (admin 候補日追加 dialog + 本人出欠入力 popover + admin status toggle dropdown)。**残 phase**: 2-C = settings の members CRUD + 凡例 chip editor + CANCELLED 行復帰 UI、3 = Discord 通知、4 = リマインダー cron + 連携。設計は `.claude/plans/todo-dazzling-locket.md` (全体) / `todo-2-phase-2-virtual-panda.md` (2-A) / `todo-2-phase-2-b-resilient-seal.md` (2-B) | 中 |
