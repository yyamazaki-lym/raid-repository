...
> **W-5 は 2026-09-07 に「発見」まで実装した (ライブ取込は未実装)。**
> どこを見るのが正しいかは固定の運用で変わる (FFLogs 上に static = guild を
> 作っているか、計測担当の個人アカウントで上げているか) ため、決め打ちに
> せず **設定で 3 択**にした: 貼られた URL のみ (既定) / guild から / 接続
> アカウントから。既定を自動発見にしないのは、guild で他コンテンツも回して
> いる固定で無関係なレポートが取り込み枠を食うため。
>
> 「練習日のみ 2 分ポーリング」は入れていない。既存の日次 cron と手動同期に
> 相乗りさせる (1 回の同期で 1 リクエスト追加) 形にとどめた — ライブ中に
> 前プルの W-1 / W-2 が出る価値は認めるが、専用のポーリング経路を持つと
> 「練習日の判定」「多重起動の防止」「レート制限」の 3 つを新たに抱える。
> まず発見経路が実運用で機能するかを確かめてから判断する。
...
| W-16 | **iCal 購読フィード** (メンバー別トークン URL、確定セッションのみ) | Sesh / Apollo / Chronicle | Google / Apple / Outlook で自動同期。既存の単発 Google カレンダーリンク (`calendar-link.ts`) と共存 | Google 側の取り込みが 8〜24 時間遅延 → 「中止・時刻変更の正は Discord 通知」と明記。Route Handler + トークン発行 | 小 | ☆ **見送り (2026-09-07)** |

> **W-16 は見送りとした (2026-09-07)。** `src/lib/calendar-link.ts` に
> 2026-08-30 時点の判断として「iCal 配信はサーバー実装が要るうえ、認証付き
> portal の予定を外部 URL で撒くことになるため採らない」と明記されており、
> 本書の再提案 (メンバー別トークン URL) はその懸念を**部分的にしか**解消
> しない: トークン付き URL は実質パスワードで、Google / Apple のサーバーが
> **認証なしで取得する**前提は変わらず、端末単位の失効もできない。
> 既存の「Google カレンダーへ 1 クリック追加」リンクで用途は足りていると
> 判断し、2026-08-30 の判断を維持する。再検討するなら、フィードに載せる
> 情報を「日時だけ」に絞ったうえで、トークンの失効 UI と合わせて設計する。
...
| UI-15 | **ボスアイコン列で進行度** (討伐順、hover で詳細) | Raider.IO ⚠️ | コンテンツ一覧のティア見出し | ステータスバッジより一覧性が高い | 小 (☆ **見送り 2026-09-07**) | アイコン素材の権利確認 (公式素材は利用規約に従う) |

> **UI-15 は見送りとした (2026-09-07)。** ボスアイコンの素材が無く、
> 用意する手段がどれも引っかかる: 公式素材 (Lodestone / ゲーム内) の
> 再配布は利用規約の範囲外で、XIVAPI 等の第三者 API 経由でも**ゲーム
> アセットの再配布**であることは変わらない。自作アイコンは 8 層分 ×
> ティアごとに描き続ける保守が発生し、「小」のコストに収まらない。
>
> 一方、この項目の狙い (ティア見出しで討伐の進み具合が分かる) は
> **既存のステータスバッジ + 2.16 で入れた進行トレンド / 実績バッジ**で
> 実質的に足りている。アイコン列にしたときの上積みは「一覧性」だけで、
> 素材の権利と保守を負う理由にならないと判断した。
>
> 再検討する条件: 権利上問題なく使えるアイコンセット (公式が配布を
> 許可した素材、または CC ライセンスの二次創作アイコン) が現れたとき。
