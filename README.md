...
- 自前作成式では **定期枠**（毎週どの曜日に活動するか）を設定でき、候補日の自動追加がその曜日だけになる。候補日ダイアログの「繰り返し」で**期間 + 曜日の一括生成**（既存の日付は飛ばす）。定期枠から外れた日には「臨時」「今回だけ」バッジが付く
...
- カードに **日別の到達度スパークライン**（直近 8 週）。タブを開かずに「今どこまで来たか」が分かる
- **難易度ラベル**と**進行モデル**（層 / フェーズ）をコンテンツごとに指定可（8.0 の新難易度のように名称が未発表でも運用できる）
...
- **軽減表 / ロット管理**: 既存 Google Sheets を iframe で全幅表示（80% スケール）
  ...
  - シート内のワークシート（層）を**層タブ**で切替。カード表示 / シート表示のどちらでも効き、`?gid=` で URL 共有できる
...
- **練習ログ**: FFLogs の pull 単位データを取り込んで表示
  ...
  - **進行トレンド**（到達度の推移グラフ + クリアまでのペースの目安）と**チーム実績バッジ**（初討伐 / ノーデス討伐 / 最速討伐 / 討伐回数）
  - フェーズ (P1〜) 表記は**絶コンテンツのみ**（零式は残 HP% で表示）。雑魚戦 (Trash Fights) は取り込み時に除外
  - 日ごとに **プル・ボックス列**（1 pull = 1 小箱、到達区間の番号入り、討伐は ✓）。日を開かずに「その日どう転がったか」が読め、箱を押すとその pull まで飛ぶ
  - **層 / フェーズの絞り込み**は 1 段のセグメントで、選択が URL に載る（`?floor=4b` / `?phase=2` を Discord に貼るとその状態で開く）
  - 日付を開くと pull 一覧 + **セッションサマリー**（拘束 / 実戦闘 / 戦闘外 / 平均プル長）。各 pull から **FFLogs の該当 fight / XIVAnalysis の解析 / 動画のその瞬間** へ 1 クリック
  ...
  - 絶は **フェーズ滞在時間**（pull ごとのフェーズ帯バーと、全 pull 合計の割合）と **各フェーズへの初到達**（そこまでの累計戦闘時間 / pull 数）
  - 零式は **各層の初討伐**（その層の累計戦闘時間 / その層の pull 数 + ティア通算 / 日時は hover）
...
「良い / 注意 / 悪い」を表す色は全画面で 1 本のスケールに揃えています（`src/lib/perf-tone.ts`）: **良い = emerald → lime → amber → orange → rose = 悪い**。練習ログの残 HP%（討伐 = 良い）・死亡数・到達度バー、出欠の記号（○ / ⏰ / △ / ×）、今週の消化チェック（消化済 / 辞退 / 未消化 N 名）、BiS の部位充足が同じ色の意味を持ちます。数値や記号は常に併記し、色だけで意味を伝えません。層・フェーズの識別色（sky → teal → violet …）は「どれか」を表す色で、良し悪しではないので別体系です。
...
> 📌 **2026-09-08 に関数を 1 本追加しました** (`category_progress_by_day`、コンテンツカードの到達度スパークライン用)。再実行していないデプロイでは**カードの線が出ないだけ**で、それ以外の機能に影響はありません。
