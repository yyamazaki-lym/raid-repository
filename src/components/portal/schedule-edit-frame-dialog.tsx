...
const SCROLL_OFFSETS = {
  ...
  //
  // 2.1 (2026-04-30 part5): 当初 2400 で運用したが、ユーザーから
  // 「登録ボタンからさらに下にスクロールが必要」との指摘。実測でコンテン
  // ツ高 ~3200px 以上 + 表示エリア ~800px なので 3600 に引き上げ。
  // 3600 は character-sheets input ページの実測高 (~3000-3400px、行数
  // で多少変動) を表示エリア (h-[92svh] ~ 720-820px) と合わせた下端到達
  // 位置を上回る値。短いページの場合は余白が増えるだけ。
  bottom: 3600,
} as const;
