...
- **2.1 (2026-05-02 part6)**: #67 `changelog.ts` (629 行 / 234 KB) を最新 1 件のみに削減 + 過去 9 リリースを `src/lib/changelog-archive.ts` に分離 + dynamic import 化 — クローズ
  ...
  - **2026-09-06 追記**: 上記運用が 2026-05 以降止まり本体が 32 件 / 420 KB に育っていたため復旧 (PR #299)。以降 `scripts/check-changelog.mjs` を CI で実行し本体 1 件を強制。同日、part の `body` (開発者向け本文) を `docs/release-notes/v<version>-<date>.md` に移し TS は `title` のみ (archive 630 KB → 37 KB)。手順は `changelog-meta.ts` の 5 点セット、書き方は `docs/release-notes/README.md`
