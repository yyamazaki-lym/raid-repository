...
export async function fetchNativeSchedule(
  ...
  // W-18 (2026-09-08): 有志練習 (任意参加) の日。列が無い旧 DB でも
  // `undefined ?? false` で従来どおり (バッジが出ないだけ)。
  const optionalByRawDate: Record<string, boolean> = {};
  for (const s of sessionRows) {
    ...
    optionalByRawDate[s.raw_date] = s.is_optional === true;
  }
