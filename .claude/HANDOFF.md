...
- **2.6 (2026-06-10)**: TODO #86 クローズ — FFLogs 連携の日次 cron 自動化 (`/api/cron/fflogs-sync`、UTC 19:00 = JST 04:00 daily) (TODO #73 follow-up) ([PR #147](https://github.com/yyamazaki-lym/raid-repository/pull/147) squash merge `f69c2c2`)
  ...
  - **ON/OFF toggle UI は本 PR スコープ外**: `app_settings.fflogs_cron_enabled` は未設定なら fail-open で動作 (= 何もせず cron は走る)、明示停止が必要なら admin が Supabase Dashboard で `value='false'` を直接 set する運用。UI Switch は将来余地として `fflogs-sync-section.tsx` に追加候補 (TODO #2 phase 4 の `native-discord-notify-section.tsx` パターン踏襲)
  ...
  - **継続観察**: UTC 19:00 (JST 04:00) の自動発火確認は **24h 観察** で別途追跡 (Vercel Functions logs の `[cron/fflogs-sync]` ログ)。観察完了 + UI Switch 追加の要否判断はユーザー判断
