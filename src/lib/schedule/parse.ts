 ...
 * - `optionalByRawDate`: W-18 (2026-09-08) — 有志練習 (任意参加) の日か
 *   (`is_optional`)。行のバッジ表示と、時刻編集 popover のチェックボックス
 *   初期値に使う。自動確定 / 催促 / 出席統計の除外は server 側で同じ列を見る。
 */
export type NativeScheduleMeta = {
  ...
  optionalByRawDate: Record<string, boolean>;
};
