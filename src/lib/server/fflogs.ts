...
export async function fetchFflogsReportsV2(
  ...
  // canonical way to fetch reports OWNED BY a specific user.
  //
  // 仕様 (実測で確定): `reports(userID:)` はその user の Public レポート
  // のみ返す。本人 OAuth でも Unlisted / Private は露出されない。
  //
  // 【最終結論 — 再検証不要 (2026-06-11, PR #179 で本番再実測済み)】
  // フィルタなし `reports()` (startTime で直近 2 年に絞り込み、25 ページ
  // × 25 件 = 625 件上限) も実測したが、結果は raw=625 / 自分名義=0 —
  // guild 共有の他人名義レポートだけが返り、自分の Private / Unlisted
  // は 1 件も含まれなかった。v2 API 経由で自分の非 Public レポートを
  // 一覧取得する方法は存在しない。Private / Unlisted の取得経路は
  // session cookie + HTML scrape (fetchFflogsReportsHtmlScrape) か、
  // メモポップオーバーでの手動紐づけのみ。
...
export type FflogsLinkResult = {
  ...
  diag?: {
    ...
    cookieUsed?: boolean;
    /** session cookie が scrape 成功により実際に削除されたか。false なら温存中 (次回連動でも使われる)。 */
