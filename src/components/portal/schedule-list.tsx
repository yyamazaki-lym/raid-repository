...
import {
  ...
  frameDeviation,
  parseRecurringDows,
  type FrameDeviation as FrameDeviationKind,
...
export function ScheduleList({
  ...
  // W-18 (2026-09-08): 有志練習 (任意参加) の日か。sync mode / 旧 DB では
  // 常に false = バッジもチェックボックスも出ない。
  const resolveOptional = (rawDate: string): boolean =>
    nativeMeta?.optionalByRawDate[rawDate] ?? false;
  /**
   * W-15 (2026-09-08): その行が定期枠から外れているか。
   *
   * 曜日は `session.dayOfWeek` (raw_date に焼き込まれた `(火)`) から取る —
   * `parsed_date` の `getDay()` は閲覧者のタイムゾーンで動きうる。
   * 時刻 override の有無は placeholder / 一括生成の既定 (NULL) と
   * 日個別設定 (NOT NULL) の差で判定する。
   */
  const resolveFrameDeviation = (session: ScheduleSession) => {
    if (mode !== "native") return "none" as const;
    const o = nativeMeta?.timeOverridesByRawDate[session.rawDate];
    const dow = dowIndexFromLabel(session.dayOfWeek);
    if (dow === null) return "none" as const;
    return frameDeviation({
      dows: frameDows,
      dow,
      hasTimeOverride: Boolean(o?.start || o?.end),
    });
  };
...
function SessionRow({
   ...
   */
  /**
   * W-15 (2026-09-08): 定期枠からの例外 (`extra` = 臨時 / `time` = 今回だけ)。
   * 定期枠が未設定なら常に `"none"` で、バッジは 1 つも出ない。
   */
  frameDeviation?: FrameDeviationKind;
  nativeTimeMeta?: {
  ...
  /**
   * W-18 (2026-09-08): 有志練習 (任意参加) の日か (`is_optional`)。
   * 行のバッジと SessionTimeEditPopover のチェックボックス初期値に drill。
   * 過去行にも出す — 「その日は任意参加だった」は振り返りで必要な情報。
   */
  isOptional?: boolean;
  /**
    ...
    >
      ...
      >
        <div className="flex items-center gap-1">
          ...
          {/* W-15 (2026-09-08): 定期枠から外れた日の印。「例外 UI が無いと
              定期化は逆効果」(調査ノートのデメリット欄) への回答で、
              定期枠を決めた瞬間に「いつもどおり」が既定になるぶん、
              そうでない日が見て分かる必要がある。 */}
          <FrameDeviationBadge deviation={deviation} />
          {/* W-18 (2026-09-08): 有志練習 (任意参加) の印。フラグを立てた日は
              自動確定・催促・出席集計から外れるので、外れていることが行を
              見て分かる必要がある (詳細は optional-session-badge.tsx)。 */}
          <OptionalSessionBadge optional={isOptional} />
