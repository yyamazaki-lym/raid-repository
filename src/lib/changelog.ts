...
export const RELEASES: ReleaseEntry[] = [
  {
    ...
    parts: [
      ...
      {
        title: "📅 スケジュール「確定」列クリック → 該当日 iframe スクロールジャンプ (TODO #44)",
        body: "スケジュール表の upcoming 行で「確定」列のセル (緑の確定バッジ or 未確定の `·`) を押すと、元サイト (NEXT_PUBLIC_SCHEDULE_URL = list view) を iframe で開き、該当日付の行近くにスクロールした状態で表示。複数日先の予定を確認するときに毎回スクロールダウンしなくて済む。\n\n**背景**: cross-origin iframe は scrollTo / postMessage できず、character-sheets は URL hash 由来のスクロールにも対応していない (1.9.14-15 で確認済)。\n\n**実装**: 1.9.15 で導入した `translateY` clipping (= iframe 上端を強制的に隠して指定 px 下から見せる) を per-date 化。`schedule-list` の確定列セルから session の upcoming index を `targetOffsetPx = 280 + (index - 1) * 36` で計算し `ScheduleEditFrameDialog` に渡す。280 = 既存の \"mid\" base (ヘッダー高)、36 = 日付行 1 行分の概算高さ。\n\n**UI**: ダイアログ右上のオフセット切替ボタンを 3-way 化 — 「該当日 / 中央 / 上」を循環。target offset が無い経路 (= ユーザー名ヘッダー / 出欠セル押下) は従来どおり 2-way (中央 / 上) で、本機能の追加で出欠セル経路の挙動は影響を受けない。URL prop が変わったら mode を初期値にリセットしてセッション間で位置が固定化されるのを防止。",
      },
      {
