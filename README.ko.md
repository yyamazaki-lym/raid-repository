...
- **정기 요일**(「매주 화 / 목 / 토」)을 설정하면 후보 날짜가 그 요일에만 자동 생성되고, 대화상자에서 **기간 × 요일 일괄 생성**도 가능. 정기 요일을 벗어난 날에는 「임시」「이번만」 배지가 붙음
...
- 카드마다 **최근 8주 진행도 스파크라인** — 탭을 열지 않고도 현재 진척을 확인 가능
...
- **연습 기록**: FFLogs 의 풀 단위 데이터를 가져와 표시 — 총 풀 / 연습 일수 / 최대 도달 / 클리어 횟수, 일별 진행 바, 각 풀에서 FFLogs / XIVAnalysis / 영상의 해당 시점으로 원클릭. 각 풀에 **전멸 원인**(가장 먼저 죽은 직업 ← 치명타 기술, 10초 이내 사망 수)과 어떤 기믹에서 무너지는지 집계. 절 콘텐츠는 **페이즈별 체류 시간**과 **각 페이즈 최초 도달**, 영식은 **층별 최초 클리어**. 하루마다 **풀 박스 열**(풀 1개 = 구간 번호가 적힌 작은 상자, 클리어는 `✓`)을 표시하고, 층 / 페이즈 필터는 선택이 URL 에 남는 세그먼트 컨트롤(`?floor=4b` / `?phase=2`). 개인 DPS 는 저장·표시하지 않으며 사망 기록에도 플레이어 이름은 없음(직업 + 기술까지)
