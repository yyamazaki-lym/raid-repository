...
| 79 | **`schedule_source_mode='disabled'` 時のデフォルトページをコンテンツページに切替** — 現状は `/` (schedule top) が `ScheduleDisabledNotice` ([src/components/portal/schedule-disabled-notice.tsx](src/components/portal/schedule-disabled-notice.tsx)) を出すだけになり、運用上ほぼ使わないトップが残り続けてしまう。schedule を使わない portal では「コンテンツページ」(カテゴリ一覧 `/category` または最初の可視カテゴリ `/category/[slug]`) を実質のホームとして見せたい。実装候補は (a) `/` の Server Component で `mode==='disabled'` のとき `redirect('/category')` に切替 + `MainTabs` の「スケジュール」タブを非表示化 / disabled 表示、(b) `MainTabs` の home link `href` を mode に応じて差し替え (admin が settings から戻せるよう `Schedule` タブは残す案もあり)、(c) `(portal)/layout.tsx` 側で mode 判定 + middleware redirect。admin が mode を sync/native に戻したいときの導線 (settings dialog をどこから開けるか) と、可視カテゴリ 0 件 fork のフォールバック (空 portal で `/category` が空表示) の設計が論点 | 小〜中 |
